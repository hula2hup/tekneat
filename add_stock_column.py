...
class Menu(db.Model):
    ...
    stock = db.Column(db.Integer, default=100)  # New stock column
    toko_id = db.Column(db.Integer, db.ForeignKey('toko.id'), nullable=False)

with app.app_context():
    # Add the new column to existing database
    with db.engine.connect() as conn:
        conn.execute(db.text("ALTER TABLE menu ADD COLUMN stock INTEGER DEFAULT 100"))
        conn.commit()
    print("Column 'stock' added to 'menu' table successfully.")
